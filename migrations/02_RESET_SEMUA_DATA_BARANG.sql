-- =============================================================================
-- LIVIRA — RESET SELURUH DATA BARANG (REVISI APLIKASI 033 / SKEMA 001 s.d. 032)
-- =============================================================================
-- PERINGATAN:
...
select
  ...
  (
    ...
    from public.audit_logs
    where entity_type in (
        'inventory', 'inventory_batch', 'disposition', 'disposition_batch',
        'research_request', 'reconciliation', 'document', 'auction_schedule', 'report'
      )
       or action like 'inventory.%'
       or action like 'process.%'
       or action like 'reconciliation.%'
       or action like 'document.%'
       or action like 'report.%'
  ) as operational_audit_after,
  (select count(*) from storage.objects where bucket_id = 'livira-documents')
    as physical_storage_files_remaining,
