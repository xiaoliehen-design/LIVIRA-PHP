-- ============================================================================
-- LIVIRA — REBRANDING, CAKUPAN DASHBOARD, DAN TARGET BONGKAR/MUAT PER KONTAINER
-- Migration 029
--
-- Jalankan file ini pada database operasional yang sebelumnya memakai prefix
-- teknis lama. Migration ini tidak mengubah data inventory. Fungsi, trigger,
-- dan index lama diganti nama secara aman agar backend LIVIRA dapat memakai
-- endpoint RPC berprefix livira_. Hak akses dan dependency PostgreSQL tetap
-- melekat karena object di-rename, bukan dibuat ulang.
-- ============================================================================

begin;

-- Rename seluruh function di schema public yang masih memakai prefix merek lama.
-- Schema auth tidak disentuh karena merupakan schema terkelola Supabase.
do $$
declare
  object_row record;
  new_name text;
...
begin
  for object_row in
    ...
    where n.nspname = 'public'
      and p.prokind = 'f'
      and position('sentra' in p.proname) > 0
    order by n.nspname, p.proname, p.oid
  loop
    new_name := replace(object_row.object_name, 'sentra', 'livira');
    select exists (
      select 1
      from pg_proc target_proc
      join pg_namespace target_namespace on target_namespace.oid = target_proc.pronamespace
      where target_namespace.nspname = object_row.schema_name
        and target_proc.proname = new_name
        and pg_get_function_identity_arguments(target_proc.oid) = object_row.identity_arguments
    ) into target_exists;
...
-- Perbaiki referensi function di dalam body yang masih menyebut prefix lama.
-- ALTER FUNCTION ... RENAME tidak menjamin referensi tekstual di body function
-- SQL/PLpgSQL ikut berubah. Tanpa langkah ini, livira_dashboard_summary() dapat
-- tetap memanggil public.sentra_inventory_dashboard_metrics() dan menghasilkan
-- error saat dashboard dimuat.
do $$
declare
  object_row record;
...
begin
  ...
  loop
    function_definition := replace(
      pg_get_functiondef(object_row.oid),
      'public.sentra_',
      'public.livira_'
    );
...
-- Rename trigger pada schema public agar audit schema dan inspeksi database
-- konsisten dengan LIVIRA. Trigger pada auth.users sengaja TIDAK di-rename:
-- auth.users adalah tabel terkelola Supabase dan ALTER TRIGGER mensyaratkan
-- kepemilikan tabel. Nama trigger lama tidak memengaruhi fungsi aplikasi; jika
-- function trigger di schema public di-rename, relasi trigger tetap terjaga via OID.
do $$
declare
  object_row record;
  new_name text;
begin
  for object_row in
    ...
    where not t.tgisinternal
      ...
      and position('sentra' in t.tgname) > 0
  ...
  loop
    ...
    if not exists (
      ...
      where target_trigger.tgrelid = object_row.tgrelid
        and target_trigger.tgname = new_name
    ) then
      execute format(
...
-- Rename index non-kritis yang masih membawa nama merek lama.
do $$
declare
  object_row record;
  new_name text;
begin
  for object_row in
    ...
    where c.relkind = 'i'
      and n.nspname = 'public'
      and pg_has_role(current_user, c.relowner, 'MEMBER')
      and position('sentra' in c.relname) > 0
    order by c.relname
  loop
    new_name := replace(object_row.object_name, 'sentra', 'livira');
    if to_regclass(format('%I.%I', object_row.schema_name, new_name)) is null then
      execute format(
...
-- Pastikan RPC bongkar/muat tersedia walaupun migration 028 lama belum pernah
-- dijalankan. CREATE OR REPLACE aman untuk database yang sudah memilikinya.
...
-- Bucket baru untuk upload dokumen setelah rebranding. File lama tetap dapat
-- diunduh karena setiap metadata dokumen menyimpan nama bucket asalnya.
