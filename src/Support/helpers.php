...
function tpl_tpp_block(mixed $value): string
{
    $location = trim((string) tpl_get($value, 'Location'));
    if ($location === '' || !tpl_truthy(tpl_get($value, 'AtTPP'))) return '';
    $facility = trim((string) tpl_get($value, 'FacilityName'));
    $status = trim((string) tpl_get($value, 'LocationStatus'));
    if (($facility !== '' && strcasecmp($location, $facility) === 0) || ($status !== '' && strcasecmp($location, $status) === 0)) return '';
    return $location;
}
